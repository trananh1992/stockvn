...
def api_get_foreign(timeStamp):
    url = 'https://fiin-market.ssi.com.vn/MoneyFlow/GetForeign?language=vi&ComGroupCode=VNINDEX&time={}'.format(timeStamp)
    r = api_request(url)
    list_buy = []
    list_sell = []
    for i in range(len(r['items'][0]['today']["buy"])):
        ticker = r['items'][0]['today']["buy"][i]['ticker']
        priceChange = r['items'][0]['today']["buy"][i]['priceChange']
        foreignNetBuyValue = r['items'][0]['today']["buy"][i]['foreignNetBuyValue']
        matchPrice = r['items'][0]['today']["buy"][i]['matchPrice']
        referencePrice = r['items'][0]['today']["buy"][i]['referencePrice']
        df = json_normalize({})
        df[''] = i
        df['ticker'] = ticker
        df['priceChange'] = priceChange
        df['foreignNetBuyValue'] = foreignNetBuyValue
        df['matchPrice'] = matchPrice
        df['referencePrice'] = referencePrice
        list_buy.append(df)

    for j in range(len(r['items'][0]['today']["buy"])):
        ticker = r['items'][0]['today']["buy"][j]['ticker']
        priceChange = r['items'][0]['today']["buy"][j]['priceChange']
        foreignNetSellValue = r['items'][0]['today']["buy"][j]['foreignNetSellValue']
        matchPrice = r['items'][0]['today']["buy"][j]['matchPrice']
        referencePrice = r['items'][0]['today']["buy"][j]['referencePrice']
        df = json_normalize({})
        df[''] = j
        df['ticker'] = ticker
        df['priceChange'] = priceChange
        df['foreignNetSellValue'] = foreignNetSellValue
        df['matchPrice'] = matchPrice
        df['referencePrice'] = referencePrice
        list_sell.append(df)

    list_buy = pd.concat(list_buy)
    list_sell = pd.concat(list_sell)
